...
\hline
House 5 &  \makecell{Out of memory \\(UNKNOWN)} & \makecell{$\leq$ 1 sec \\(SAT)} & \makecell{$\leq$ 1 sec \\(SAT)} & \makecell{6 sec \\ (SAT)}\\
\hline
\hline
\end{tabular}
\end{scriptsize}
...
A comparison with a specialized simulation tool was also performed, although some of the tool's limitations, mainly related to not allowing battery autonomy setup or oversizing the battery bank by HOMER Pro, restricted the comparison between automated verification tools and simulation tools.
