...
We thus develop algorithms and the corresponding tools; moreover, we evaluate them with several case studies, in a comparative with specialized simulation tools, supported by real data from deployed PV systems.
...
Note that it is out of our scope to perform code modification on the verifiers or the solvers used during the automated verification or automated synthesis steps of our approach. This thesis intend to create front-end applications to the verifiers and solvers (that it will be the back-end). That decision is based on the fact that we can be verifier-independent, and evaluate different back-ends, now and in the future, always searching for the best performance and soundness.
...
Related with the proposed \textbf{automated verification} method, this thesis makes three main contributions. 
...
\textbf{Secondly}, we propose a variant CEGIS method of synthesis with striking differences of how the {\sc Synthesize} and {\sc Verify} phases from original CEGIS work (without solution candidate vector and using an incremental and iterative loop to reach the optimal cost solution); and 
\textbf{Thirdly}, experimental results with seven case studies show that our approach qualitatively outperforms state-of-the-art simulation tool: our solution is far detailed and closer to real PV systems than the solution presented by simulation.
...
~\autoref{chap:publications} presents a list of papers that we submitted to international journals and conference related to the use of automated verification method. It covers the years from 2015 to 2019, related to the Ph.D. process.
...
It is worth mentioning that we decided, in agreement and with the consent of the coordination of the postgraduate program, that this document would be written in active voice and entirely in English. The choice of the passive voice comes from the choice of the English language, which in the studied thesis always always presented in active voice. On the other hand, the choice of language comes from the possibility that the research may lead to further developments and activities, without the need for translation.