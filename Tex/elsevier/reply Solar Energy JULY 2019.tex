...
The authors hold real load curve and information collected from monitoring system using Raspberri Pi deployed  \textcolor{red}{at House 1, House 2, House 3 and House 4}; however, based on the fact that the automated verification is mathematical and not visual,  \textcolor{red}{(automated verifiers do not present curves or plots; they present the result of FAIL or SUCCESS analysis of coded system, with the counterexample detailing the status of the variables that cause a FAIL in the analysis), so it seems that this comparison is not adequate}.

The information from temperature and solar irradiance from every house location, used by the simulation and formal verification tools, come from historical data collected from real stations, which are available in Weather Base and Energy Plus web sites, \textcolor{red}{as presented at Section "References"}. Considering that the idea of our study is to show the effectiveness of the proposed verification method just after the design phase (i.e., before deployment), we believe that it is a plausible procedure \textcolor{red}{and not using weather base stations in each place. Even because the fact that House 1, House 2, House 3, and House 4 (the ones in a riverside indigenous community) do not have weather station available to collect data and the information must be approximated to Manaus data (the closest municipality with historical data and regular measurement of data). Because of that, we decide to include this issue at the Section "5.4 Threats to Validity". We thank you for that specifically comment}.
\end{resposta}
...
The authors hold real load curve and information collected from monitoring system using Raspberri Pi deployed  \textcolor{red}{at House 1, House 2, House 3 and House 4}; however, based on the fact that the automated verification is mathematical and not visual,  \textcolor{red}{(automated verifiers do not present curves or plots; they present the result of FAIL or SUCCESS analysis of coded system, with the counterexample detailing the status of the variables that cause a FAIL in the analysis, as described in Section 4 of the paper), so it seems that this comparison is not adequate}.
