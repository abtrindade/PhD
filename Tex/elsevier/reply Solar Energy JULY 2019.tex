...
\begin{resposta} 
We thank the reviewer fot this insightful comment. Indeed, it summarizes what was intended to be studied in this manuscript. Additionally, we emphasize that the main focus of our study relies on the validation process of solar PV systems instead of the mathematical modelling, which was already investigated by prior studies over the last two decades; this particular information was carefully included in the revised abstract, introduction and conclusion sections since we concluded that it was unclear in the previous version of our manuscript. Regarding Figure 2, we fixed it as indicated by the reviewer. Moreover, we applied the same procedure to voltage V, in order to keep consistency throughout our manuscript.
...
\begin{resposta} 
We appreciate these comments; we actually realized that our manuscript was unclear. We should highlight that our main research focus is on the validation process of the stand-alone solar PV system instead of the mathematical modelling, which was already investigated in prior studies over the last two decades. Moreover, we compare the results of using automated verification with simulation; real PV systems were employed just to validate the results presented by both approaches (model checking and simulation). Note that the automated verification and simulation tools are intended to be used during the design of the PV system, i.e., before actually buying or deploying the equipment. This information was carefully included in the revised abstract, introduction and conclusion sections since we concluded that it was unclear in the previous version of our manuscript.

The authors hold real load curve and information collected from monitoring system of four (in five) PV systems/cases \textcolor{red}{could you please identify them here? which one does not have such a information?}; however, based on the fact that the automated verification is mathematical and not visual (curves/plots), it seems that our comparison is not an adequate way \textcolor{red}{why not? explain the reason here}.

The information from temperature and solar irradiance from every site, used by the simulation and formal verification tools, come from historical data collected from real stations, which are available in Weather Base and Energy Plus web sites \textcolor{red}{shall we provide the website here?}. Considering that the idea of our study is to show the effectiveness of the proposed verification method just after the design phase (i.e., before deployment), we believe that it is a plausible procedure \textcolor{red}{to....}. Note that the sites from four case studies (i.e., the ones in a riverside indigenous community) do not have weather station available to collect data and the information must be approximated to Manaus data (the closest site with historical data) \textcolor{red}{what are the consequences? shall we state this limitation in the threats to validity section?}.
\end{resposta}
...
\begin{resposta} 
We thank the reviewer for this comment. We did as suggested, i.e., we changed the section title to ``Verification and Simulation Results" since the results are related to both verification and simulation tools. The data collected from dwellers with real PV systems were used just to confirm flaws and complains about the equipment deployed in the field. The idea was not to present wave forms because how automated verification works is different from that of simulation or testing \textcolor{red}{why is it different? explain here... please do not point to the answer of another reviewer}.
\end{resposta}
...
When we work with waveform, we are performing testing of simulation \textcolor{red}{``testing of simulation'' sounds weird. Could you please explain it further here?}, and that is different of automated verification, which is a mathematical proof (of correctness or flaw). Based on the fact that the authors have the five cases deployed in field and with monitoring system in four those PV systems, it is possible to add waveforms to the work. However, the use of this resource seems that is not an adequate way of comparative \textcolor{red}{why not? could you please explain here?}. However, we included details about the estimated load curve of each case study in the revised version of our manuscript (cf. Section $4$), which is represented by a vector with $24$ elements (one to each hour of the day).
\end{resposta}
\end{quote}
