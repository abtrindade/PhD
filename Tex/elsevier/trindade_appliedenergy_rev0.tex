...
\textcolor{red}{However, in our variant CEGIS method proposed at this paper, there are four particular differences related to traditional CEGIS: (1) there is not test vectors and every candidate is generated during the run-time at the {\sc Synthesize} phase and sent to {\sc Verify} phase; (2) if the {\sc Verify} phase is not successful, then a new candidate is generated by {\sc Synthesize} and (3) the lower bound of {\sc Verify} phase is incremented in order to pursuit the lowest cost; (4) therefore, there is not the refinement from the {\sc Verify} back to {\sc Synthesize} phase (a new counterexample is not added to the {\sc inputs} set) because a FAIL during {\sc Verify} just discard the candidate, that can be feasible in the next iteration with a new lower bound.}
