...
and CPAchecker\footnote{Command-line: \$ scripts/cpa.sh -heap 64000m -config config/bmc-incremental.properties -spec config/specification/sv-comp-reachability.spc filename.c} were used as our verification engine to compare our approach effectiveness and efficiency. Worth to mention that "incremental" ESBMC with solver Z3 was tried\footnote{Command-line: \$ esbmc filename.c -\phantom{}-no-bounds-check -\phantom{}-no-pointer-check -\phantom{}-unwind 100 -\phantom{}-smt-during-symex -\phantom{}-smt-symex-guard -\phantom{}-z3} as an alternative to use less computing memory. Simulation tool HOMER Pro version 3.13.1 was used during comparative.
...
Comparing the results between the formal synthesis with CPAchecker and HOMER Pro, it is easy to notice that mostly of the results were very similar, in terms of technical solution and cost, with exception of case 2 and case 7. In case 2, the costs presented by HOMER Pro were 54\% higher than the automated synthesis tool, probably because the operation and maintenance costs assumed by our automated synthesis tool were underestimated to that specific load. In general, the size of the PV panels were bigger in HOMER Pro than with CPAchecker, and that discrepancy is not easy to address without some real systems. Even considering that PVs and inverters were kept at the same cost in both methodologies (simulation and formal synthesis), HOMER Pro have more parameters to be set and a different mathematical model adopted. Just to compare the results with the real world, the authors had four PV systems deployed and monitored since June 2019 at a riverside community in Amazon State in Brazil, with similar demand presented by cases 1, 4, 5, and 6, always with a 3$\times$325 W (3S) panels and 4$\times$220 Ah (2S-2P) lead-acid batteries. That solutions is more close to the result presented by CPAchecker than HOMER Pro, and that answer partially \textit{RQ1}.
