...
Only a niche market a few years ago, solar PV systems are now becoming a mainstream electricity provider, with an increase of approximately 50\% from 2016 to 2017 in terms of new installations of PV~\citep{EPIA}. %However, this growth is not driven by stand-alone systems.
...
The most popular software tools are summarized in Table~\ref{table:softwares}: PVWatts, SAM, HOMER, RETScreen, and Hybrid2~\citep{Pradhan,Swarnkar,NRELDobos,NRELBlair,Mills}.
...
As highlights, only HOMER and Hybrid2 perform off-grid system with battery backup analysis. \textcolor{red}{Hybrid2 is not supported anymore, being in disuse since that not work on Windows platforms later than Windows XP~\citep{UMASS, Pinho}.} %all the tools perform solar photovoltaic analysis; 
...
A stand-alone PV system is illustrated in Fig.\ref{fig:blockdiagram}. The PV generator is a semiconductor device that can convert solar energy into DC electricity\textcolor{red}{, with high dependence on two weather variables from the site, where the system is deployed: solar irradiance $G$ and temperature $T$}. 
For night hours or rainy days, power stored in batteries can be used and it implies the presence of a charge controller~\citep{Hansen}. The PV arrays produce DC and therefore when the PV system contains an AC load, a DC/AC conversion is required (inverter). The AC load dictates the behavior of AC electrical load from the house that will be fed by the system. \textcolor{red}{In this pictured modular structure, every element produces/consumes current $I$ and voltage $V$, as illustrated by their physical magnitudes, where $pv$ means photovoltaic, $bat$ is battery, $dc$ is direct, and $ac$ is alternating signals.}
%
...
\centering
\caption{\textcolor{red}{Block diagram for a typical stand-alone PV system, adapted from~\citep{Hansen}}.}
...
\centering
\caption{\textcolor{red}{1-diode equivalent PV cell/panel circuit model, adapted from~\citep{Cubas}.}}
...
\noindent where $I_{ph}$ is the photo-current delivered by the constant current source; $I_{0}$ is the reverse saturation current corresponding to the diode; $N$ is the number of series-connected cells; %($N=1$ in a single cell configuration); 
...
%
Here, the model adopted uses only manufacturer's data without empirical tests~\citep{Copetti}. % and allows finding relations among voltage, current, state of charge and temperature. 
%
...
%\end{figure}
%
\textcolor{red}{Note that the steps described here are carried out shortly after the design of the solar PV system, as soon as the equipment and its specifications are defined, i.e., before buying and deploying them, as a safe approach to ensure that the project will succeed.}
...
In Step 1, the PV input data \textcolor{red}{(e.g., load power demand and load energy consumption)} 
...
\textcolor{red}{In our study, we estimated load curves based on survey and visiting performed in June of 2017, and the the estimated load curve was converted as vectors with 24 positions, one to each house of the day (in Watts):}
%
\begin{itemize} 
\item \textcolor{red}{House 1: [118, 118, 118, 46, 46, 46, 95, 95, 170, 170, 296, 242, 242, 95, 95, 95, 95, 95, 342, 288, 288, 288, 288, 118];} 
\item \textcolor{red}{House 2: [136, 136, 136, 136, 136, 136, 67, 67, 184, 184, 184, 184, 184, 67, 67, 67, 67, 67, 253, 253, 253, 253, 253, 136];} 
\item \textcolor{red}{House 3: [113, 113, 113, 113, 113, 113, 67, 67, 217, 97, 97, 97, 97, 97, 97, 97, 97, 97, 263, 113, 113, 113, 113, 113];}
\item \textcolor{red}{House 4: [207, 207, 207, 135, 135, 135, 66, 66, 161, 161, 233, 253, 248, 66, 66, 66, 66, 66, 302, 317, 322, 302, 302, 207];} 
\item \textcolor{red}{House 5: [45, 16, 16, 16, 16, 16, 0, 0, 0, 72, 72, 222, 150, 150, 0, 0, 72, 72, 814, 814, 814, 742, 742, 16].}
\end{itemize}
...
%
%----------------------------------------------
\section{\textcolor{red}{Verification and Simulation Results}} 
\label{sec:results}
%----------------------------------------------
%
...
\hline
\textcolor{red}{Power peak (W)}& 342  & 253  & 263 & 322 & 814 \\
\hline
\textcolor{red}{Power surge (W)}& 342 & 722 & 732 & 896 & 980\\
\hline
\makecell{\textcolor{red}{Consumption}\\ \textcolor{red}{(kWh/day)}}& \textcolor{red}{3.9} & \textcolor{red}{3.6} & \textcolor{red}{2.5} & \textcolor{red}{4.3} & \textcolor{red}{4.88} \\
\hline
...
\hline
\end{tabular}
\textcolor{red}{Legend: (S): Series; (P): Parallel.}
\end{scriptsize}
\end{table}
%------------------------------------------
...
\label{sec:setup}
%------------------------------------------
\textcolor{red}{We aim} to answer two research questions:
%
\begin{enumerate}
...
\textcolor{red}{Note that the results presented here depend on the computer's processor and memory, the version of each software verifier (i.e., CBMC, ESBMC, CPAchecker), the parameters passed by the command-line, and the implemented model (software created by authors) to be solved. Additionally, any change from HOMER Pro to another simulation tool can also influence the measured time to obtain results.}
...
Nevertheless, we have also identified \textcolor{red}{five} threats to the validity of our results that can further be assessed.
...
As future work, we will \textcolor{red} {expand the number of case studies (preferentially in different countries), develop the code of a general purpose simulation tool to include in the comparative, and also consider} other types of renewable energy and even hybrid ones to allow our method to verify typical rural electrification.
%the proposed method will be extended to start from a list of commercial equipment, where each equipment is verified and the final solution, which satisfies the project specification, is found, thus leading to an optimum sizing of PV systems. 
%We will also consider other types of renewable energy and even hybrid ones to allow our method to design and verify typical rural electrification.
%
