...
Here we develop and evaluate a sound, automated synthesis approach to obtain optimal sizing of stand-alone PV systems. We propose a variant of the counterexample guided inductive synthesis (CEGIS) approach with two phases linking the technical and cost analysis. 
We advocate that the application of CEGIS to obtain optimal PV sizing has various advantages if compared to off-the-shelf optimization tools available in the market.
...
Experimental results demonstrate that we can produce an optimal solution within an acceptable run-time. We also compare our CEGIS approach with a state-of-the-art commercial tool over real PV systems to show its effectiveness.
