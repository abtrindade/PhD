...
\begin{center}
\textbf{\LARGE Synthesis of Solar Photovoltaic Systems: Optimal Sizing Comparison} \\
\hfill \break
...
We must estimate the total power that will be demanded from the PV panels, as defined by Eq. ~\eqref{eq:Pminpanel}. Where $Insolation$, also called solar irradiation or solar exposure, is expressed in terms of $kWh/m^{2}$ per day and depends on the site where the PV system will be deployed. An empirical factor of $20$\% for losses is considered~\cite{Pinho}, because $1.25 = 1 \mathbin{/} (1 - 0.2)$.
