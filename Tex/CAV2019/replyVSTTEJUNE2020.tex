...
According to the reviewers' comments, our manuscript was carefully revised%, and all detailed changes are highlighted in blue color, along with the original text
. The revision included the Abstract, Introduction, Background, Synthesizing Optimal Sizing, Results, and Conclusions sections. A highlighted version of the entire paper is available in \url{https://tinyurl.com/y9scqv6g} if the reviewers would like access.
...
\begin{resposta} 
We thank the reviewer for these insightful comments and suggestions. We had to revise parts of the Background and the Synthesizing Optimal Sizing sections to address the right conference attendants' profile. The PV sizing modeling was changed to a higher level of explanation, with a flowchart in page 6 (Fig.4). The proposed synthesis algorithm followed the same principle in page 8. Besides that, in case of interest in the complete and detailed mathematical sizing of PV systems, that we removed from the paper, we created a link to a 5-page PDF online file that can be accessed.
\end{resposta}
...
\begin{resposta} 
We thank the reviewer for this insightful comment and suggestion. You are correct; Reviewer \#1 also provided the same observation. We had to revise parts of the Background, and the Synthesizing Optimal Sizing sections to address the right conference attendants profile. Our manuscript was revised, and the PV sizing modeling was changed to a higher level of explanation, including a flowchart in Section 2 (cf. Figure 4 at page 6). In Section 3 (page 8), the synthesis algorithm followed the same idea. Besides that, in case of interest in the complete and detailed mathematical sizing of PV systems, that was removed from this revised paper, we created a link to a 5-page PDF complementary file that can be accessed.
\end{resposta}
...
\begin{resposta}
We agree with the reviewer. We missed the details about why those values are taken. Considering that the new paper version has a higher level explanation for the PV sizing, we removed this part and transferred it to the complementary PDF text. Therefore, we revised the corresponding online text to: ``The $DOD$ is usually $25$\% for lead-acid batteries, and $80$\% for lithium batteries and represent an intrinsic and empirical feature of those type of batteries [Pinho and Galdino, 2014] in order to guarantee its lifetime''.
\end{resposta}
...
\begin{resposta}
We have updated the entire paragraph in the complimentary online file to explain the meaning of these variables. ``The short circuit reference information ($I_{sc,ref}$) from the manufacturer's solar panel must be corrected because field temperature ($T$) is higher than nominal or laboratory temperature ($25^{\circ}C$), and the PV system is temperature dependent, as described by Eq. (9) for the ambient short-circuit current from the PV panel. $G$ is the solar irradiance from the deployment site, and $G_{ref}$ is defined as $1000$ $W/m^{2}$ by the manufacturers.''
\end{resposta}
...
A detailed account is given of both modeling parameters, evaluation method and evaluation results. The details, covered in the paper, contain
...
\begin{resposta} 
We thank the reviewer for these insightful comments and suggestions. Those are the same observations did by Reviewer \#1 and \#2. We had to revise parts of the Background, and the Synthesizing Optimal Sizing sections to address the right conference attendants profile. Our manuscript was revised, and the PV sizing modeling was changed to a higher level of explanation, including a flowchart in Section 2 (cf. Figure 4 at page 6). In Section 3, the synthesis algorithm followed the same idea (page 8). Besides that, in case of interest in the complete and detailed mathematical sizing of PV systems, that was removed from this revised version of the paper, we created a link to a 5-page PDF complementary file that can be accessed as well.
\end{resposta}
