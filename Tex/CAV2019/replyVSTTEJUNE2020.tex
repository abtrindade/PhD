...
\begin{resposta} 
We thank the reviewer for this insightful comment and suggestion. Indeed, we had to revise the Background, and the Synthesizing Optimal Sizing sections in order to address the right conference attendants profile. The paper was reviewed and the PV sizing modelling was changed to a more higher level of explanation. Different parts of the above mentioned sections in blue indicate where we changed/reviewed the text.
...
\begin{resposta} 
We thank the reviewer again for this insightful comment and suggestion. Indeed, we had to revise every section of the paper in order to address the specific issues highlighted by the reviewer......
...
We model these PV sizing constraints using floating point arithmetic. It is just an approximation of the real numbers. However, in this work, we are not concerned with calculating the rounding error, which is negligible when we consider the size of the physical quantities and the variables adopted.
...
\begin{resposta} 
We thank the reviewer for this insightful comment and suggestion. He/she is right and it was the same observation did by Reviewer \#1. In order to correct this issue, we had to revise the Background, and the Synthesizing Optimal Sizing sections in order to address the right conference attendants profile. The paper was reviewed and the PV sizing modelling was passed to a more higher level of explanation. Different parts of the paper in blue indicate where we changed/reviewed the text.
...
\begin{resposta}
The reviewer is right, we missed the details about why those values are taken. Therefore we reviewed the text to: "The $DOD$ is usually $25$\% for lead-acid batteries, and $80$\% for lithium batteries and represent an intrinsic and empirical feature of those type of batteries [Pinho and Galdino, 2014] in order to guarantee its lifetime".
\end{resposta}
...
\begin{resposta}
We have updated the entire paragraph to explain the meaning of these variables as follows: ``The short circuit reference information ($I_{sc,ref}$) from the manufacturer's solar panel must be corrected because field temperature ($T$) is higher than nominal or laboratory temperature ($25^{\circ}C$), and the PV system is temperature dependent, as described by Eq. (9) for the ambient short-circuit current from the PV panel. $G$ is the solar irradiance from the deployment site, and $G_{ref}$ is defined as $1000$ $W/m^{2}$ by the manufacturers.''
\end{resposta}
