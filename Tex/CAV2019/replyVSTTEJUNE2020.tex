...
\begin{resposta} 
\textcolor{red}{where do we address this comment? Please indicate the section.}
We use four nondeterministic variables to hold four matrices with complete datasheet information from an equipment item. We have four variables and four matrices: one to PV panels, one to batteries, one to the inverter, and one to the charge controller. Those nondeterministic variables are used during the search for the feasible solution and controlled by the statements \textbf{assume}. The candidate solution is proposed in the phase {\sc Synthesize} of the technique. For the {\sc Verify} phase, we performed a linear search for the lowest cost, and the result is based on FAIL or SUCCESS check from a cost that is linearly incremented in this model. The model checking is used during this {\sc Verify} phase. If the process produces FAIL, then an optimal solution is found. If SUCCESS is provided, there is no feasible solution from the equipment list provided by the technique. We added a future work at the end of the CONCLUSIONS section, related to the possibility of speeding up the {\sc Verify} phase through binary search or even using specific optimization targeted solvers (as $\nu Z$).
\end{resposta}
