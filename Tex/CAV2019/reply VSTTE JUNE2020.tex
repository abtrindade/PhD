...
(7)       Can the author present the main waveform of the PV system in the steady state operation and under failure or any environmental change? wave-forms such as: Input power, VMPP, IMPP, AC load  
...
When we work with waveform, we are performing testing or simulation, and that is different of automated verification. We consider that this comment (number $7$ in this Reply Letter) is related to the experimental question done by the same reviewer in (6). As a result, we expect that the explanations given in (6), together with the consequent updated manuscript, can cover any further concerns about this particular issue. 
\end{resposta}
\end{quote}
