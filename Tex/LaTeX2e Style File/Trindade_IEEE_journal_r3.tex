...
This is the first study to apply a formal verification technique to formally check the design of a stand-alone solar PV system. In summary, this paper makes two original contributions. % Firstly, we describe a modular modeling of each component of a PV system by means of mathematical models that can be encoded into fragments of first-order theories supported by software model checkers. Secondly
Firstly, we propose an automated verification method that formally checks the design of a given PV system using incremental model checking based on Satisfiability Module Theories (SMT). Secondly, experimental results show that this proposed approach can find subtle design errors in PV systems, which are not easily detected by other approaches based on simulation. 
...
$a$ is the ideality or quality factor ($a=1$ for ideal diodes and between $1$ and $2$ for real diodes); $V_{T}$ is the thermal voltage ($ V_{T}=k_{B}T/q $); $ k_{B} $ is the Boltzmann constant ($ 1.3806503\times10^{-23}J $); $T$ the temperature of cell in Kelvin; $q$ is absolute value of the electron's charge ($ -1.60217646\times10^{-19}C $).
...
%
Firstly, we need to correct the energy consumption estimated to the load ($E_{consumption}$), which is carried out by ~\eqref{eq:Ecorrected}, where the efficiency of batteries ($\eta_{b}$), controller ($\eta_{c}$), and the inverter ($\eta_{i}$) are considered~\cite{Pinho}.
\begin{equation}
\label{eq:Ecorrected}
...
the other variables were discussed previously in Section~\ref{sec:PVmodel} and ~\ref{sec:BATmodel}.
%
...
We have also used the SMT solver Z3 version 4.7.1~\cite{DeMoura}. %The experiments were performed without a predefined timeout.
