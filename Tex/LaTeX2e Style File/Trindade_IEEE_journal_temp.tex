...
\begin{abstract}
With declining costs and increasing performance, the deployment of renewable energy systems is growing in both developed and developing countries, with particular emphasis on stand-alone solar photovoltaic systems where grid extension is not feasible. Tools to evaluate the design and validate electrification projects are available; however, those tools are based on simulation that do not cover all the aspects of the design space. Automated verification using formal methods, in the other hand, is proven to be a reference technique to program verification, and any kind of system can be verified by formal method, including the most complexes. At this paper, each component of a stand-alone solar photovoltaic system, as solar panel, charge controller, battery, inverter, and electric load, were coded as a mathematical model, and a algorithm using automated verification was created in order to validate its design from a formal verification perspective. Case studies, from real photovoltaic systems deployed in five different sites, ranging from 700 W to 1,200 W, were used to evaluate the proposed technique, always comparing the results with the specialized simulation tool HOMER Pro. Data from practical application prove the results obtained by the proposed approach, where specific conditions that lead to failures in a photovoltaic solar system are only detected with the verification tool.
...
Here it was evaluated the most popular ones, which are PVWatts, SAM, HOMER, RETScreen, and Hybrid2 \cite{NRELDobos}, \cite{NRELBlair}, \cite{Swarnkar}, \cite{Pradhan}, \cite{Mills}.
...
Table~\ref{table:softwares} summarizes the off-the-shelf tools employed in this study, where only Hybrid2 does not have technical support; HOMER and Hybrid2 perform off-grid system or battery backup analysis; %all the tools perform solar photovoltaic analysis; 
additionally, HOMER and RETScreen include economical analysis or even optimization-sensitive analysis. RETScreen and HOMER have a free web-based version, but they have limited resources: it is not allowed to save the projects or even upload data from manufacturers. In other hand, commercial version of those tools, called RETScreen Expert and HOMER Pro, are available only for Microsoft Windows and the annual subscription range from US\$504.00 to US\$657.00.
...
Equation \eqref{eq:1diodemodel} is used to express currents and voltages at each key point of the characteristic curve from a PV cell \cite{Villalva}.
...
Validation is the process of determining whether a design meets the needs of the user, whereas verification is the process of determining whether a design meets a set of requirements, specifications, and regulations~\cite{ClarkeHV18}.
...
% 
%Verification procedure is an intelligent \textcolor{red}{What do you mean by intelligent?} exhaustive search of the state space of the design \cite{ClarkeHV18}. In addition, formal verification is a systematic approach that applies mathematical reasoning to obtain guarantees about the correctness of a system \cite{Forejt2011}. One successful method in this domain is model checking \textcolor{red}{please cite here the reference given above}.
 % 
...
However, there is a main disadvantage of model checking: the state explosion problem. In order to solve the problem of state explosion, many different techniques were developed at the last decades. One of the first major advances was symbolic model checking with binary decision diagrams (BDDs). In this approach, a set of states is represented by a BDD instead of by listing each state individually, which is often exponentially smaller in practice. Another major advance is the partial order reduction, which exploits the independence of actions in a system with asynchronous composition of processes. A third major advance is counterexample-guided abstraction refinement, which adaptively tries to find an appropriate level refinement, not burdened with irrelevant detail that slows down verification \cite{Clarke2012}. However, one of the promising is the Bounded Model Checking (BMC). 
...
\textcolor{red}{I didn't understand this sentence... it is completely broken...} Simply put, instead of the ESBMC find the defect of a software or if a hardware does not meet the some requirements, the tool will verify if the requirements of the solar PV project are met. Detailing the condition of violation, i.e., the state of each variable related to the system. 
...
Algorithm~\ref{alg:verification-algorithm} describes the pseudo code used to perform the automated verification. \textcolor{red}{Can you please briefly explain this algorithm?}
