...
\begin{abstract}
With declining costs and increasing performance, the deployment of renewable energy in off-grid systems is growing in both developed and developing countries, with particular emphasis on solar photovoltaic systems. Electrification of rural areas can improve the education standard, health conditions, and also the life of this population. In addition, depending on the country geography, using renewable energy sources in off-grid mode is a feasible solution if compared to grid extension. Tools to aid the design and validation of rural electrification projects are available; however, there is no software that applies formal verification to evaluate rural electrification projects. This paper aims to build a framework to apply an automated verification method to mathematical models of a solar photovoltaic system to validate its design from a formal verification perspective. Experimental results show that the proposed approach can find situations where the sized solar photovoltaic system fails to met the requirements demanded to electrify a house, and that simulation made with commercial tools can not find the same failure. Data from practical application of solar systems prove the results obtained by the proposed approach.
\end{abstract}
...
However, those tools are based only in simulation models. The use of simulation for verification is problematic, because is necessary to ensure the complete coverage of operating conditions, scenarios, and system inputs. 
...
% ideas will be 
proposed framework is done with the efficient SMT-based bounded model checker (ESBMC) %tool that uses the front-end of the C Bounded Model Checker (CBMC) or the clang-based front-end, 
 which permits to describe the PV system as a C/C++ language code.
  
The results prove that the framework is more efficient in different case studies to find not proper functioning of PV systems than simulation with off-the-sheld tool. To the best of our knowledge, this is the first work to use a
%n SMT-based 
