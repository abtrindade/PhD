...
If the SMT formula is shown to be satisfiable, a counterexample is presented; if the formula is find to be unsatisfiable, there are no errors up to the unwinding bound, and this result is presented. 
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%5%
...
As Step 01, the PV input data (load power demand, load consumption of energy, the PV system project sizing data, and the manufacturer's data-sheet information), the formulas to check the sizing project, the mathematical model, the limits of the weather non-deterministic variables, are all written as an ANSI-C code \cite{ANSI2018}. At Step 02, occurs the sizing check of the PV system to make sure that the components followed accepted international requirements toward a well designed PV project.
...
Information related to average temperature ($T$) and solar irradiance ($G$), maximum and minimum annual, are passed to algorithm at lines 7 to 10 using the feature of non-deterministic variable from ESBMC to explore all the possible states from the variables and the \textit{assume} macro to limit the values considering a range. 
...
every 24h-day was considered as a time-step of 1 hour, and it was divided in 2 parts: (a) one that there is PV generation, during daylight, with a duration in hours depending on each site; and (b) one the includes all the rest of the day (without PV generation). Therefore the proposed methodology depends on the  specific data about the solar irradiation levels in order to define the average amount of hours of PV generation.

Following, the model from PV generator is used at the function call of line 11, to produce the voltage and current considering the actual $G$ and $T$. Following, related to every hour considered, the conditional \textit{if-else-endif} statements from lines 12, 17, 23 and 28, will perform the charge and discharge of batteries according the PV generation, the updated state of charge of batteries, the house's load and the set-up information of the system.
After this process is started the battery autonomy verification, from line 31. Based on the fact that won't be PV generation after a given time of the day, the algorithm will only discharge the batteries until a new charging process (at the next day) to start. The formal verification is guaranteed by \textit{assert} macro to specific variables of the model, according lines 27 and 35.
...
Experimental setup of automated verification tool: all experiments were conducted on an otherwise idle Intel Core i7-2600 (8-cores), with 3.4 GHz and 64 GB of RAM, running Ubuntu 18.04.1 LTS 64-bits. Concerning the SMT tool, ESBMC v5.1 was used with the incremental parameter in order to reduce the memory usage. At this paper, it was used the Z3 solver \cite{DeMoura} with version 4.7.1.
