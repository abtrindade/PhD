...
\begin{abstract}
With declining costs and increasing performance, the deployment of renewable energy systems is growing faster. Particular attention is given to stand-alone solar photovoltaic systems in rural areas or where grid extension is not feasible. Tools to evaluate electrification projects are available; however, those tools are based on simulation that do not cover all the aspects of the design space. Automated verification using model checking has proven to be an effective technique to program verification. However, any kind of system can be verified by model checking, including the most complex. Here each component of a stand-alone solar photovoltaic system, as solar panel, charge controller, battery, inverter, and electric load, were encoded into a mathematical model, and a algorithm using model checking features was created in order to validate its design from a formal verification perspective. Case studies, from real photovoltaic systems deployed in five different sites, ranging from 700W to 1,200W, were used to evaluate the proposed technique, always comparing the results with the specialized simulation tool HOMER Pro. Data from practical applications show the effectiveness of the obtained results, where specific conditions that lead to failures in a photovoltaic solar system are only detected with the proposed verification method.
...
Related to the standardized input language to SMT solvers (SMTLIB logic format), there is a resource called \textit{assertion stack}. An assertion, in SMT solvers, is a constraint on the variables in formula that must hold if the formula is satisfiable \cite{Morse2015}. New assertions can be added to or old assertions removed from this stack, depending on the evaluated value of variables. This enables the solver to learn from previous checks, optimizing the search procedure and potentially eliminating a large amount of formula state space to be searched, because it solves and discart data during the process, incrementally. This technique is called "Incremental SMT" \cite{Morse2015}. The process of retraction to build and deconstruct the formula allow to reduce the overhead of memory, mainly when the verified system is complex and the computing platform do not have large amount of memory to deal with all the design space state.
%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%5%
...
%\item[RQ2] \textbf{(performance)} How does our approach compare against other existing tools?
%
We have evaluated five case studies in total. Related to the results of the automated verification and the evaluation if the results are correct: (a) the 1,200 W PV system (house 5) failed during the sizing check. The number of panels was \textit{incorrect}; in particular, the counterexample provided by our verification method indicated 3 PV panels in parallel and the sized project has 2 in series and 2 in parallel. That verification took 63.3 hours to be performed. Surveying the owner of the 1,200 W system it was identified that, in fact, the system mostly of the time do not met the battery autonomy (mainly when all the loads are turned on). That behavior is expected because the system was purchased as an off-the-shelf solution and not as a specific design for the electrical charges of the house; (b) Related to the four 700 W PV systems of houses 1, 2, 3, and 4, the sizing check was successful during automated verification, but there was found flaws related with the battery autonomy, when SOC reached a empty-battery level. The automated verification identified the flaws (for all houses) right after the first night-discharge cycle, before the solar system start to recharge the batteries. The proposed tool took 409.3 hours to find this flaw at house 1; 611.2 hours to house 2; 615.8 hours to house 3, and 620.8 hours to house 4. This possible flaws were confirmed with the dwellers that owns the systems by interview: at least once a mouth is usual the system to turn off, normally in raining or clouds days, reaching the situation described in Step 3 of \ref{table:controller}; and after the sun rises, the systems returns to normal operation. 

The same five case studies were evaluated by HOMER Pro. The result of simulation shows that the project restrictions were met by four 700 W PV systems (house 1, 2, 3 and 4), without any indication of sizing error or even performance. The case study that was unsuccessful during simulation was the 1,200 W (house 5); however, without any indication about the failures of this PV system. All the simulations took less than 5 seconds (each) to be performed with HOMER Pro.

Despite the divergence of results for the houses 1, 2, 3 and 4, comparing our automated tool with HOMER Pro, it is evident that the information collect from the dweller shows that the automated verification has the correct evaluation of the PV system. House 5 presented flaws from both tools, however only the automated verification showed which system project error was responsible for the flaw (number of PV panels).

It is worth to mention that a PV design always uses daily average values of sun hours to each site, with impact at the PV components. Those hours are based on historic data and, in field, it is not unusual to find days where that number of hours was not reached, because of weather. The season has impact, as the fact that the case studies are from the Rain Forest, where the clouds are always present. With that in mind, the identified flaws at houses 1, 2, 3, and 4, are justified once again.
%
