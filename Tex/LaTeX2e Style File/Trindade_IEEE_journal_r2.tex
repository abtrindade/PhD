...
\begin{abstract}
With declining costs and increasing performance, the deployment of renewable energy in off-grid systems is growing in both developed and developing countries, with particular emphasis on solar photovoltaic systems. Electrification of rural areas can improve the education standard, health conditions, and also the life of that population. In addition, depending on the country geography, using renewable energy sources in off-grid mode is a feasible solution if compared to a grid extension. Tools to aid the design and validation of rural electrification projects are available; however, there is no software that applies formal verification to evaluate rural electrification projects. This paper aims to apply an automated verification method to mathematical models of a solar photovoltaic system to validate its design from a formal verification perspective. Data from practical application of solar systems prove the results obtained by the proposed approach, where specific conditions that cause fails of the photovoltaic solar system are only detected with the verification tool when compared with a simulation commercial tool.
...
Related to solar PV systems, the project requirements, as state of charge of batteries and power demand, beside weather conditions, as solar irradiance and ambient temperature, are translated as part of the computer program and automated verified during the formal process. The process of model checking reports in which conditions the system does not met the user requirements or whether it will fail due to weather conditions, which aids to improve the project itself.  
...
%n SMT-based 
verification tool to evaluate a stand-alone solar PV systems, with weather conditions variation. The mains contribution of this paper are the automated verification of the PV system, and the modular modeling of each component of the PV system, which permits to change the mathematical models, besides changing the type of energy generation in the future to another renewable source, for example.
...
the configuration that is most common nowadays is the regulated stand-alone system with battery and AC load. This configuration, which is the target of this paper, includes PV array, batteries, charge controller, inverter, and AC load.
