...
%---------------------------------------------------
\textit{Model precision:} each component received a mathematical model, and the precision of the proposed tool depends on the precision of that model. Probably a evaluation in a laboratory to validate the model could give more reliability to the proposed tool.

\textit{Time step:} The computing complexity is a issue, but the time step of one hours can be evaluated to approximate the algorithm of the real world, where a solar irradiance and ambient temperature can change in minutes.

\textit{Case studies:} The studies were performed only at the municipality of Manaus, at the south hemisphere. A more complete evaluation can be performed if other places around the world could become case studies.
