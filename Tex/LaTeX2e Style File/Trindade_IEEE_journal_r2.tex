...
\begin{abstract}
With declining costs and increasing performance, the deployment of renewable energy systems is growing in both developed and developing countries. Particular attention is being given to stand-alone solar photovoltaic systems in rural areas or where grid extension is not feasible. Tools to evaluate the design and validate electrification projects are available; however, those tools are based on simulations that do not cover all the aspects of the design space. Automated verification using model checking, however, has proven to be an effective technique to program verification. Here each component of a stand-alone solar photovoltaic system, as solar panel, charge controller, battery, inverter, and electric load, are encoded into a mathematical model, and a software model checking algorithm was developed to verify its design from a formal perspective. Case studies, from real photovoltaic systems deployed in five different sites, ranging from $700$ W to $1,200$ W, were used to evaluate the proposed technique, always comparing the results with the specialized simulation tool HOMER Pro. Data from practical applications show the effectiveness of the obtained results, where specific conditions that lead to failures in a photovoltaic solar system are only detected with the proposed verification method.
...
The SAT or SMT solvers would search the model for conditions (value of variables) that would make the formula satisfiable. If a SAT or SMT solver finds a substitution for the formula/function then the substitute induces a counterexample and is said satisfiable.
In other words, is satisfiable only if the verified system contains errors.  
%
...
%
ESBMC's architecture can be summarized in three stages: (I) a front-end that can read and compile C/C++ codes, where the formal specification of the system to be verified is first treated; (II) preprocessing steps to deal with the representation of the code, control flow and unwinding of loops, and the simplification of the algorithm, aiming to reduce the computational complexity; and finally (III) the solver, where all the constraints and properties of the system to be verified are encoded into SMT format and solved.
%
...
If the SMT formula is shown to be satisfiable (SAT), a counterexample is presented; if the formula is find to be unsatisfiable (UNSAT), there are no errors up to the unwinding bound. 
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%5%
...
%
The flowchart of the proposed automated verification tool is illustrated in Fig.~\ref{fig:flowchartgeneral}. 
As Step 1, the PV input data (e.g., load power demand and load consumption of energy), the formulas to check the sizing project, the mathematical model, the limits of the weather non-deterministic variables, are all written as an ANSI-C code~\cite{ANSI2018}. In Step 2, occurs the sizing check of the PV system to make sure that the components accepted international requirements toward a well designed PV project.
\begin{figure}[h]
...
\end{figure}
The weather variables (solar irradiance and ambient temperature by default) will be explored in all their states by the ESBMC verification engine, in Step 3, based on maximum and minimum values from the site, where the PV system will be deployed. As a consequence, all the formulas of the mathematical models used will also be updated. In addition, depending on one of the desired properties of the system, as autonomy of the batteries, the offer of the energy, or even the power supply from the system to check, whether it is not accomplished (said satisfied), ESBMC accuses a failure, and the diagnostic counterexample shows in which condition the property violation (system fail) happened. 
...
and properties %($P$) 
from the PV system that were provided by the user, and the tool will verify automatically if the PV system requirements are met. If the return is a failure (verification satisfiable), then the tool details the condition of violation, i.e., the state of each variable related to the system. That information can be used as a feedback to improve the PV system project. However, if the verification succeeds (said UNSAT or unsatisfiable), there is no failure, therefore the PV system will present its intended behavior at the field.
%
%
%---------------------------------------------------------------------
...
Algorithm~\ref{alg:verification-algorithm} describes the pseudocode used to perform the automated verification. Line 1 indicates a function call that performs the size checking of the entire PV system: using \eqref{eq:NTPmin}, \eqref{eq:NTP}, \eqref{eq:NPSmin}, \eqref{eq:NPS}, \eqref{eq:NPP}, and \eqref{eq:NPPmin} to verify the PV panel; using \eqref{eq:Cbank}, \eqref{eq:Nbtotal}, and \eqref{eq:batcheck} to verify the batteries; using \eqref{eq:vcvsystem}, \eqref{eq:icmin}, and \eqref{eq:icicmin} to verify the charge controller; and using \eqref{eq:vindc}, \eqref{eq:voutac}, and \eqref{eq:invcheck} to verify the inverter. The verification is performed by the standard \textit{assert} macro from ANSI-C programming language to each equation. The argument to \textit{assert} must be \textit{true} when the macro is executed; otherwise, the program aborts and prints an error message. If there is no fail, the algorithm goes on and the batteries are assumed to have SOC of 80\% (Line 5).
...
Experimental setup of automated verification tool: all experiments were conducted on an otherwise idle Intel Core i7-2600 (8-cores), with 3.4 GHz and 64 GB of RAM, running Ubuntu 18.04.1 LTS 64-bits. Concerning the SMT tool, ESBMC v5.1 was used with the incremental parameter to reduce the memory usage. At this paper, it was used the Z3 solver \cite{DeMoura} with version 4.7.1.
...
We performed five case studies: (a) four concerning a PV system (700 W inverter, with 48h autonomy) deployed at four different houses at an indigenous community (GPS coordinates 2$^{o}$44'50.0"S 60$^{o}$25'47.8"W) sited nearby Manaus (Brazil), with each house having a different power demand (house 1 = 253 W, house 2 = 263 W, house 3 = 283 W, and house 4 = 501 W); (b) and one case concerning a system deployed as an individual system in Manaus (GPS coordinates 3$^{o}$4'20.208"S 60$^{o}$0'30.168"W), supporting 915 W of the house's load (house 5 with 1,200 W inverter, and autonomy of just 6 h). The annual average temperature ($T$) in Manaus is from 23$^{o}$C to 32$^{o}$C; and irradiance ($G$) varies from 274 W/m$^{2}$ to 852 W/m$^{2}$ when there is sunlight (that information is inputted at Lines 9 and 10 of Algorithm~\ref{alg:verification-algorithm}). Another characteristic of Manaus, based on historic data of solar irradiance, is related of the fact that only during 8 hours of the day there is PV generation, from 8:00h to 16:00h (that information is a demand of Algorithm~\ref{alg:verification-algorithm} as well).
...
We have evaluated five case studies in total using the HOMER Pro tool and our automated verification tool. Related to the HOMER Pro, the simulation, based on NASA data from the deployed systems (temperature and solar irradiance), shows that the restrictions were met by four systems \textcolor{red}{which four system? can you please identify each one by a number? they should also be described}, without any indication of sizing error or even performance. The case study that was unsuccessful during simulation was the 1,200 W (house 5); however, without any indication about the failures of this PV system. All the simulations took less than 5 seconds (each) to be performed.

However, related to the results of the automated verification, the 1,200 W PV system (house 5) failed during the sizing check. The number of panels was \textit{incorrect}; in particular, the counterexample provided by our verification method indicated 3 panels in parallel and the sized project has 2 in series and 2 in parallel. That verification took 63.3 hours to be performed. Surveying the owner of the 1,200 W system, we identified that, in fact, the system mostly of the time does not meet the battery autonomy (mainly when all the loads are turned on). However, it is understandable that this problem occurs since the system was purchased as an off-the-shelf solution and not as a specific design for the electrical charges of the house.

Related to the four 700 W PV systems, the sizing check was successful during automated verification, but flaws were found in all of them, all related with the battery autonomy, when SOC reached levels below 75\%. The automated verification identified the flaws right after the first night-discharge cycle, i.e., before the solar system starts recharging the batteries. The proposed method took 409.3 hours to find an error in house 1, and the maximum of \textcolor{red}{456XXX} hours to house 4. This functional problem \textcolor{red}{what is a functional problem?} was confirmed with all the four dwellers that use the system: at least once or twice a mouth is usual the system to turn off, normally during raining days or with more clouds in the sky, and after the sun rises the system returns to normal operation.
