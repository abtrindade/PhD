...
\begin{abstract}
With declining costs and increasing performance, the deployment of renewable energy in \textcolor{red}{What is the difference between off-grid and stand-lone? In the title you mention stand-alone and in the abstract you mention off-grid... please keep a standard...} off-grid systems is growing in both developed and developing countries, with particular emphasis on solar photovoltaic systems. Electrification of rural areas can improve the education standard, health conditions, and also the life of that population. In addition, depending on the country geography, using renewable energy sources in off-grid mode is a feasible solution if compared to a grid extension. \textcolor{red}{half of your abstract is about the context... you should reduce it a bit further..} Tools to aid the design and validation of rural electrification projects are available; however, there is no software that applies formal verification to evaluate rural electrification projects. This paper aims to apply an automated verification method to mathematical models of a solar photovoltaic system to validate its design from a formal verification perspective. Data from practical application of solar systems prove the results obtained by the proposed approach, where specific conditions that lead to failures in a photovoltaic solar system are only detected with the verification tool if compared to a simulation tools.
...
However, those tools are based on running experiments in simulation models. And simulation has the advantage of be cheap (when compared to test in real systems) and can be employed before the system is declared concluded, however with the drawback of an incomplete coverage, because the verification of all possible iterations and potential failures of systems is rarely pissible or feasible \cite{Clarke2008}.
...
%Thus, such problems as incompleteness, ambiguities, and inconsistencies, which normally are discovered only in later stages of the design, can be detected in advance, as described by \cite{Trindade}. 
 And finally, algorithms can verify the system model by systematically exploring all their states. That way, the formal proof can verify if the requirements are met by the system.

Formal methods, in the other hand, provide facilities for the modeling of each component of any complex system \cite{Hall1990}. It allows for developing models for each component of the system. At this paper, a mathematical model of each component of a stand-alone PV system, as panel solar, charge controller, batteries, inverter, and electrical load are created. The behavior of each system can be analyzed and observed with the help of these formal models, as the joint operation of the components, which in this case represent the operation of the solar PV system itself. A key benefit to this approach is that it helps in the detection of faults and flaws in the design phase of system development, thereby considerably improving system reliability \cite{Akram2018}.
...
Related to solar PV systems, the project requirements, as battery autonomy and power demand, beside weather conditions, as solar irradiance and ambient temperature, are translated as part of the computer program and automated verified during the formal process. The model checking tool reports in which conditions the system does not met the user requirements or whether it will fail due to weather conditions, which aids to improve the project itself.  
...
In mostly of previous studies, the evaluation of PV systems, concerning the meeting of user requirements, were performed by software simulation using MATLAB/Simulink \cite{Benatiallah2017}, \cite{Samrat2014}, and \cite{Natsheh2012} or HOMER Pro \cite{Lamnadi2017}. Some work was done toward the formal modeling of power smart grids \cite{Akram2018}, and to maximize the power point of solar panels \cite{Driouich2017}, however without to perform the automated verification and restrict to solar panels or smart grids. 

In addition, recent research that joint the formal verification to solar energy, has attempted to formalize and implement a formal study of large population of PV panels where the focus have been the modeling of the dynamics of PV panels and their interaction with the grid, without batteries \cite{Abate2017}; or to model a PV system in Modelica, to verify the maximum power point of solar panels with the use of Jmodelica tool \cite{Driouich2018}. Both restrict to PV panels, and not including batteries, inverters, and charge controllers.
...
%n SMT-based 
verification tool to evaluate a stand-alone solar PV systems. %, with weather conditions variation. 
The mains contribution of this paper are the automated verification of the PV system, and the modular modeling of each component of the PV system, which permits to change the mathematical models, besides changing the type of energy generation in the future to another renewable source, for example.
...
%\end{itemize}
Specifically concerning the energy need for remote rural areas of developing countries or places where the grid extension is not possible or feasible, 
...
the configuration that is most common or mode indicated is the regulated stand-alone system with battery and AC load. This configuration is the focus of this paper.
...
%-------------------------------------------------------------------
%
Considering the focus of this paper, only HOMER remains for a comparative with the proposed automated verification technique of this paper. %All tools need some parameters inherently from manufacturer's catalog.
...
The mathematical modeling of the PV system is based on modular blocks, as illustrated in Fig.\ref{fig:blockdiagram}. It is identified the PV generator, batteries, charge controller, inverter, and AC load. 

The PV generator, which can be a panel or an array, is a semiconductor device that can convert solar energy into DC electricity. At the diagram, there are two variables that depend on the site where the system is deployed and from the weather (solar irradiance $ G $ and temperature $ T $). For night hours or rainy days, one needs to hold batteries, where power can be stored and used. The use of battery as a storage form implies thus the presence of a charge controller \cite{Hansen}, \cite{Mellit}. The PV arrays produce DC and therefore when the PV system contains an AC load, a DC/AC conversion is required. That converter is called of inverter. And the AC load dictates the behavior of AC electrical load from the house that will be feed by the system.
%
...
Where the reference state (STC) of the cell is given by the solar irradiance $ G_{ref}=1000 W/m^{2} $, and the temperature $ T_{ref}=298.15 K (=25^{o}C) $; $ \mu_{I} $ is the short-circuit current temperature coefficient ($A/K$) %and corresponds to the photocurrent obtained from a given PV cell working at reference conditions 
...
Besides the model verification, there is the prior stage of project sizing check, based on manufacturer's data and information from the sizing and the site. 
...
Particularly, the total number of panels is series ($ N_{PSmin} $) and parallel ($ N_{PPmin} $) are given by (\ref{eq:NPSmin}) and (\ref{eq:NPPmin}), respectively. With the check done by (\ref{eq:NPS}) and (\ref{eq:NPP}). $ V_{system} $ is the DC voltage of the bus, normally 12, 24 or 48 V.
\begin{equation}
\label{eq:NPSmin}
...
%
Most of the created models were based on lead-acid batteries. That kind of battery is the most common used in PV application, owing to their relative low cost and wide availability \cite{Copetti}. 
...
Where $ C_{10} $ means 10h of rated capacity, which is standard on the manufacturer's data-sheet, $ \Delta T $ is temperature variation ($ \Delta T=T-T_{ref} $, $ T_{ref}=25^{o}C $), $ SOC $ or state of charge indicates how much electric charge is stored in the cell at given time. Mathematically, is the ratio between the present capacity and the nominal capacity (in $ Ah $, provided by manufacturer). If $ SOC=1 $, then the battery is totally charged; and if $ SOC=0 $, the battery is fully discharged.  %, defined by Equation \ref{eq:SOCbat}.
%\begin{equation}
%\label{eq:SOCbat}
...
Where $ V_{in} $ is the voltage across the PV array, $ I_{in} $ is the output current of PV array, $ V_{out}=V_{b}=V_{system} $ is the  DC bus voltage, and $ I_{out} $ is the output current from the converter.
