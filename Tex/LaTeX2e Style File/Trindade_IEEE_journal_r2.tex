...
\begin{abstract}
With declining costs and increasing performance, the deployment of renewable energy in off-grid systems is growing in both developed and developing countries, with particular emphasis on solar photovoltaic systems. Electrification of rural areas can improve the education standard, health conditions, and also the life of that population. In addition, depending on the country geography, using renewable energy sources in off-grid mode is a feasible solution if compared to a grid extension. Tools to aid the design and validation of rural electrification projects are available; however, there is no software that applies formal verification to evaluate rural electrification projects. This paper aims to build a framework to apply an automated verification method to mathematical models of a solar photovoltaic system to validate its design from a formal verification perspective. Experimental results show that the proposed approach can find situations where the sized solar photovoltaic system fails to meet the requirements demanded to electrify a house, and that simulation made with commercial tools can not find the same failure. \textcolor{red}{Data from practical application of solar systems prove the results obtained by the proposed approach... this last sentence should be stronger to emphasise your main contribution... maybe you could update this after obtaining the experimental results...}
\end{abstract}
...
\IEEEPARstart{A}{ccording} to \cite{Coelho}, there are presently 1.3 billion people with no access to electricity worldwide. 
...
However, those tools are based on running experiments in simulation models. And simulation has the pros of be cheap and be employed before the system is declared concluded, however with the cons of an incomplete coverage, because the verification of all possible iterations and potential failures of systems is rarely pissible or feasible \cite{Clarke2008}.

Formal methods based on model checking offer great potential to obtain a more effective and faster verification in the design process \cite{Clarke2008}.  
%with behavior that is, in principle, well determined. 
 Using this technique, systems are specified as programs using mathematical logic, which constitutes the intended (correct) behavior. Then, one can try to give a formal proof or otherwise establish that the program meets its specification. %, as defined by \cite{Trindade}. 
%
...
%Thus, such problems as incompleteness, ambiguities, and inconsistencies, which normally are discovered only in later stages of the design, can be detected in advance, as described by \cite{Trindade}. 
 Algorithms can verify the system model by systematically exploring all their states. 

At this paper, mathematical models of each component of a stand-alone PV system, panel solar, charge controller, batteries, inverter, and load, are translated as a single software program. Those programs are viewed as mathematical objects, and the use of 
...
and then report in which conditions the system does not meet user requirements (as electrical load, power demand) or whether it will fail due to weather conditions and/or battery autonomy, for example.  
...
% ideas will be 
proposed tool is carried out by means of the efficient SMT-based bounded model checker (ESBMC) %tool that uses the front-end of the C Bounded Model Checker (CBMC) or the clang-based front-end, 
 which allows one to formally verify the PV system as a C program.
 
Formal methods provide facilities for the modeling of each component of any complex system \cite{Hall1990}. It allows for developing models for each component of the system. The behavior of each system can be analyzed and observed with the help of these formal models. A key benefit to this approach is that it helps in the detection of faults and flaws in the design phase of system development, thereby considerably improving system reliability \cite{Akram2018}.

In previous studies, 
%%%%%%%
%%%%%%%%
\textcolor{red}{This section needs more work. You should describe here studies related to the design, verification or validation of PV systems and not about software model checkers!}

%The CBMC, it is notable its prominence through international awards received in competitions. 
%In the works r
Related to the CBMC, much of the research is focused on the improvement of the tool (performance mainly) \cite{Kroening}, without addressing, however, the issue of PV system validation.
% For example, in \cite{Schrammel2017}, CBMC received an extension to perform verification of embedded software. 

%ESBMC was developed to perform bounded model checks on both sequential and concurrent programs using a range of SMT solvers, and has a proven track of bug finding in real-world applications \cite{Cordeiro}, \cite{Ramalho}. The tool also implements a technique to prove the correctness of (some) unbounded programs, the k-induction algorithm; this approach has been applied to a large number of benchmarks and has produced more correct results than similar competing tools \cite{GadelhaSBMF}, \cite{GadelhaK}.
Related to ESBMC, it is possible to perform the partitioning of hardware-software (optimization) \cite{Trindade}. And it is possible to use ESBMC to check the stability of digital control systems \cite{Bessa}. In addition,  there is the approach to synthesize safe digital feedback controllers for physical plants \cite{Abateetal2017}. However, the use of ESBMC to verify PV systems is unprecedented. 

%Concerning modeling and simulation of 
Concerning electrical energy systems,  there is the use of agent-base model approach to evaluate the effect of loads, shedding, grids, wind power generation, and smart grids, in a dynamic integral multi-scale case study \cite{Kremers}. However, it was restrict to simulation, without the formalism of automated verification.
%that is a software development approach for the simulation of complex systems, without the formalism of automated verification. 

%In the other hand, the use of formal verification related to modeling and validation or even simulation of power systems is a young theme of research.
% According \cite{Abateetal2014}, where articles from seminars were issued at this thematic, the actual focus is on optimization and control of microgrids, HVAC systems (diagnostics and maintenance), models of generic wind turbine generation, modeling of transmission networks, and verification of smart grids. 
In addition, recent research has attempted to formalize and implement a formal study of large population of PV panels.% (very common in European Union). 
 The focus have been the modeling of the dynamics of PV panels and their interaction with the grid, without batteries \cite{Abate2017}.
%
...
%
%The framework provided at this paper outlines the requirements, the process and the mathematical modeling to perform automated formal verification of a PV off-grid system, using the satisfiability modulo theories (SMT)-based verification method.
 
The remainder of this paper is organized as follows: Section II gives the background about solar PV systems, design and validation of PV systems, and the mathematical modeling. Section III presents the automated verification technique. The methodology is presented at section IV. Section V is devoted to the results. Section VI presents the conclusion and describes future work.

%--------------------------------------------------------
\section{Solar Photovoltaic System }
%--------------------------------------------------------

\textcolor{red}{This section is too short! You should move it to another big section...}
...
the configuration that is most common nowadays is the regulated stand-alone system with battery and AC load. This configuration includes PV array, batteries, charge controller, inverter, and AC load.
...
Here it was evaluated the most popular tools, as PVWatts, SAM, HOMER Pro, RETScreen Expert, and Hybrid2.
...
Table~\ref{table:softwares} summarizes the off-the-shelf tools employed in this study, where only Hybrid2 does not have technical support; HOMER Pro and Hybrid2 perform off-grid system or battery backup analysis; %all the tools perform solar photovoltaic analysis; 
additionally, HOMER Pro and RETScreen Expert include economical analysis or even optimization-sensitive analysis. However, only paid version of those tools have all the features \textcolor{red}{which features?}, and they run only on Windows operating system. RETScreen and HOMER have a free web-based version, but they have limited resources.
...
Considering the focus of this paper, only HOMER remains for our comparative evaluation. All tools need some parameters inherently from manufacturer's catalog \textcolor{red}{is this the correct term? to size the system}.
...
After that, the potential solution is analyzed in another tool to validate or even optimize the solution. Therefore, that is the challenge of this work, i.e., to prove that is possible to use automated verification to validate an off-grid PV solution and to prove that this verification is more efficient and complete than other state-of-the-art tools \textcolor{red}{cite them here...}. The comparative evaluation between the HOMER Pro and the proposed verification tool is presented in Section \ref{sec:results}.
...
The modeling of the PV system is based on modular blocks, as illustrated in Fig.\ref{fig:blockdiagram} \textcolor{red}{you should explain each element of the figure}. The modular structure facilitates the modeling of the other system structures and replacement of elements such as a DC load instead of an AC load. 
...
The performance of photovoltaic systems, %that is, the output current/voltage curve ($I-V$ curve), 
is usually studied using an equivalent circuit model, as described in the available literature \cite{Yatimi}, \cite{Saloux}, \cite{Mellit}. This equivalent circuit consists of a current source with one or two diodes connected in parallel, and up to two resistors, one connected in parallel and the other one in series, to take into account energy losses in this model \cite{Cubas}. 
...
The 1-diode model, shown by Fig. \ref{fig:equivckt}, whose equation to relate the output current, $I$, to the output voltage, $V$, is described in (\ref{eq:1diodemodel}). 
\begin{equation}
...
Equation (\ref{eq:1diodemodel}) is used to write down expressions for currents and voltages at each key point of characteristic curve from a PV cell.
...
Where $ T_{air} $ is the ambient temperature, $NOCT$ is the nominal operating cell temperature (in $^{o}$C) that is find at the PV manufacturer's data-sheet, and $G$ is the solar irradiance ($ W/m^{2} $) of the place where the PV system is deployed.

Furthermore, (\ref{eq:I0}) allows the saturation current ($ I_{0} $) to be expressed as a function of the cell temperature \cite{Villalva}. 
...
Using the maximum power point current (\ref{eq:Pm}) and the saturation current in the reference temperature given by (\ref{eq:I0}), the diode quality (or ideality) coefficient is determined by (\ref{eq:a}).
\begin{equation}
\label{eq:a}
...
First is necessary to correct the energy consumption that was estimated to the load ($E_{consumption}$). That is done by (\ref{eq:Ecorrected}) \cite{Pinho}. Where the efficiency of batteries ($\eta_{b}$), controller ($\eta_{c}$), and the inverter ($\eta_{i}$) are considered.
\begin{equation}
\label{eq:Ecorrected}
E_{corrected} = \dfrac{E_{consumption}}{ \eta_{b} \eta_{c} \eta_{i} }
\end{equation}

The total minimum number of solar panels necessary($ N_{TPmin} $) is calculated by  (\ref{eq:NTPmin}), and the check is done with (\ref{eq:NTP}), where the sized number of panels ($ N_{TP} $) must be greater than the result from (\ref{eq:NTPmin}).
\begin{equation}
\label{eq:NTPmin}
...
Particularly, the total number of panels is series ($ N_{PSmin} $) and parallel ($ N_{PPmin} $) are given by (\ref{eq:NPSmin}) and (\ref{eq:NPPmin}), respectively. With the check done by (\ref{eq:NPS}) and (\ref{eq:NPP}). $ V_{system} $ is the DC voltage of the bus, and is a design decision, normally 12, 24 or 48 V.
\begin{equation}
\label{eq:NPSmin}
...
Several models have been presented in the literature \cite{Copetti}, \cite{Manwell93}.
%, and \cite{Manwell94}. 
...
Here, the model adopted \cite{Copetti} uses manufacturer's data and allows finding relations among voltage, current, state of charge and temperature. 

The discharge voltage equation is shown in (\ref{eq:bat_Vd}). 
...
For the charging process, however, the parameters are presented in (\ref{eq:Vcbat}).
%
\begin{multline}
\label{eq:Vcbat}
...
%
%
Besides the model verification, there is also the prior stage of project sizing check, as did to the solar panel. First is necessary to define the total capacity of the battery bank, as shown by (\ref{eq:Cbank}).
\begin{equation}
\label{eq:Cbank}
...
Following, is done the total (minimum) number of batteries, as show by (\ref{eq:Nbtotal}). Moreover, (\ref{eq:batcheck}) perform the final sizing check, considering the number of batteries in series ($ N_{BS} $) and the number of batteries in parallel ($ N_{BP} $) that were established to the project.
\begin{equation}
\label{eq:Nbtotal}
...
The output power ($ P_{out} $) of DC-DC converter is given by (\ref{eq:poutcont}). 
\begin{equation}
\label{eq:poutcont}
P_{in} = P_{out}
\end{equation}

Assuming that the efficiency of the controller ($ \eta_{c} $) is a manufacturer's data, from (\ref{eq:poutcont}) is possible to reach (\ref{eq:potcont}).
\begin{equation}
\label{eq:potcont}
...
One more time, some steps must be done to check the sizing project of the controller, prior the verification phase. Initially the controller must meet the voltage requirement of the PV system, as shown by (\ref{eq:vcvsystem}). 
\begin{equation}
\label{eq:vcvsystem}
V_{c} = V_{system}
\end{equation}

Following, the short circuit reference information from the manufacturer's solar panel must be corrected to the cell temperature, as shown by (\ref{eq:iscamb}).
\begin{equation}
\label{eq:iscamb}
I_{sc,amb} = I_{sc,ref} \times \left[ 1 + \eta_{I} \times (T-25) \right] 
\end{equation}

The controller must meet the maximum current from the PV array (\ref{eq:icmin}), and (\ref{eq:icicmin}).
\begin{equation}
\label{eq:icmin}
...
Where $ I_{DC} $ is the current required by the inverter from the DC source in order to be able to keep the rated voltage on the AC side, $ V_{DC} $ is the input voltage to the inverter delivered by the DC source (PV panel or battery),  $ V_{AC}  $ and $ I_{AC} $ are the output voltage and current, respectively, and $ cos \varphi $ can be find from the inverter's data-sheet.
...
Verification procedure is an intelligent exhaustive search of the state space of the design \cite{Clarke2008}. In addition, formal verification is a systematic approach that applies mathematical reasoning to obtain guarantees about the correctness of a system \cite{Forejt2011}. One successful method in this domain is model checking.
 % 
...
\end{itemize}
Specifically at the $FALSE$ situation, the algorithm returns a counterexample that is useful as diagnostic of the system, in order to discover in which situation the model is violated. That is the most important advantage of the use of model checking \cite{Clarke2008}. 
...
The model checking problem can be defined as \cite{Clarke2008}: 
...
, as illustrated in Fig.\ref{fig:flowchartgeneral}
...
Algorithm 1 describes the pseudo code used to perform the automated verification.
 ...
 \\ \textit{LOOP Process}
  ...
  \STATE assume ($Gmin \leq G \leq Gmax$) \COMMENT {restricting the values of $G$}
  \STATE assume ($Tmin \leq T \leq Tmax$) \COMMENT {restricting the values of $T$}
  \STATE $Imax, Vmax \leftarrow PVgenerationMODEL (G,T)$ 
  \\
  \COMMENT {Now, testing if battery is empty:}
  \IF {($SOC \leq SOC_{limit}$)} 
    \STATE assert (PV panel is generating energy) \COMMENT {if there is no enough solar energy generation, then FAIL}
  ...
  \ELSE
    ...
    \\
    \COMMENT {the last case: batteries are charged but the panels are not generating}	  
  \ENDIF
...
\STATE $AutonomyCount \leftarrow 1$
 \WHILE {$AutonomyCount \leq autonomy$}
  ...
  \\
  \COMMENT {Perform the 08:00h-16:00h \textbf{for}-LOOP as above}
  %%\ENDFOR
  \ENDWHILE
 \RETURN $(\,)$ 
 \end{algorithmic} 
...
%
%The framework presented here establishes the automated verification and the project sizing check. 
It was shown that, beside the time-consuming issue of the proposed framework, it overcomes the results of off-the-shelf tool that only simulate the results. The framework was successful in find sizing errors and to indicate in which conditions, in terms of temperature and solar irradiation, a PV system can fails.
...
\textcolor{red}{You should revise all references according to the journal guideline.}
