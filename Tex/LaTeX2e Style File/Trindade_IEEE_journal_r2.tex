...
The SAT or SMT solvers would search the model for conditions (value of variables) that would make the formula satisfiable. If a SAT or SMT solver finds a substitution for the formula/function then the substitute induces a counterexample.  
%
...
If the SMT formula is shown to be satisfiable, a counterexample is presented; if the formula is find to be unsatisfiable, there are no errors up to the unwinding bound. 
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%5%
...
\begin{figure}[h]
\includegraphics[width=0.4\textwidth]{flowchart_verification3.png}
\centering
\caption{Flowchart of the proposed automated verification of PV systems. \textcolor{red}{replace 01, 02 and 03 by 1, 2, 3.}}
\label{fig:flowchartgeneral}
\end{figure}

The weather data (e.g., solar irradiance and ambient temperature by default) will be evaluated by the ESBMC verification engine, in Step 3, based on maximum and minimum values from the site, where the PV system will be deployed. In addition, depending on one of the requirements of the system, as autonomy of the batteries, or the offer of the energy, or even the power supply from the system to check whether it is not accomplished or satisfied, ESBMC accuses a failure, and the diagnostic counterexample shows in which condition the fail/violation happened. 
...
and properties %($P$) 
from the PV system that were provided by the user, and the tool will verify automatically if the requirements are met. If the return is a failure, then the tool details the condition of violation, i.e., the state of each variable related to the system. That information can be used as a feedback to improve the PV system project. However, if the verification succeeds, the PV system will present its intended behavior at the field.
%
%
%---------------------------------------------------------------------
