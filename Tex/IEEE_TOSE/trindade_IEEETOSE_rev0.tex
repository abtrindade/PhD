...
%-----------------------------------------------------------
Here we explain how to size a stand-alone solar PV system with their respective mathematical model, and what type of criteria or technique is possible to apply for the optimal sizing of PV systems. Furthermore, we present the theory about program synthesis,
...
A PV system is illustrated in Fig.\ref{fig:blockdiagram}. It identifies the PV generator, batteries, charge controller, inverter, and AC load. The PV generator, which can be a panel or an array, is a semiconductor device that can convert solar energy into DC electricity. 
...
to critical period solar energy method~\cite{Pinho} and adopting MPPT (Maximum Power Point Tracking) charge controller, which is the most common one in practice, among other sizing techniques and possibilities. 
...
%------------------------------------------------------
\section{Synthesizing optimal sizing of stand-alone solar PV systems}
\label{sec:Method}
%------------------------------------------------------
...
%---------------------------------------------------------------------------
\subsection{Comparative Tools} 
%---------------------------------------------------------------------------

The start-of-art verification tool, CPAchecker\footnote{Command-line: \$ scripts/cpa.sh -heap 64000m -config config/bmc-incremental.properties -spec config/specification/sv-comp-reachability.spc filename.c} were used as our verification engine to compare our approach effectiveness and efficiency. The Simulation tool HOMER Pro version $3.13.1$ was used for comparative purpose.
