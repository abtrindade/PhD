...
In this section, we present the theory and methodology created to obtain the optimal sizing of stand-alone solar PV systems through program synthesis. 

\textcolor{red}{Fig.~\ref{fig:optimization} illustrates how to obtain the optimal sizing of a stand-alone PV system, showing the traditional techniques (manual and simulation), and including the proposed automatic synthesis. Worth to mention that the input information is the same for all the methods (weather data, detailed equipment and price information, design requirements, as load curve and power demand, and design assumptions), with the difference that in automated synthesis is possible to define the bound $k$ to restrict the design-space search. Related to the output presented by the techniques, all produce a successful or fail result considering a feasible technical solution that has the lowest cost. However when done by simulation we get a report or graphical result; in the other hand, the automated synthesis technique, that is a mathematical reasoning of a model, presents a counter-example with the optimal solution stored in variables. Furthermore, as it will be showed in this section, the design-space coverage during the optimal sizing search is more complete when using automated synthesis.}
...
supply probability, which can make the analysis more \textcolor{red}{precise by considering the dynamic of the weather characteristics over the year or by electric load changes over time, 
...
of applicability \textcolor{red}{in other isolated areas of the world, as India and China, for example}; ($3$) to deploy at the field some PV systems 
sized using our synthesized results to validate it\textcolor{red}{, because a comparative with a real system may be more reliable than comparing with a simulation tool}.
