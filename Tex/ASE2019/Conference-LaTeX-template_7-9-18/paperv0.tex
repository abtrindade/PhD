...
The modeling of a PV system is typically based on modular blocks, as illustrated in Fig.\ref{fig:blockdiagram}. %It identifies the PV generator, batteries, charge controller, inverter, and AC load. 
...
%
Only CPAchecker was able to present the optimal sizing of the two case studies: the result came in $50.2$ minutes and $37.2$ minutes, respectively. 
The violation of line 209 indicated in Table~\ref{tab1} is the $assert$ of line 28 from Algorithm~\ref{alg:verification-algorithm}. The results were tested by manual PV sizing and were sound (\textit{RQ1}). %, linking a feasible technical solution with the lowest cost possible, considering the equipment that were inputted to the code. 
CBMC and ESBMC produced \textit{UNKNOWN} result (\textit{timeout} or \textit{out of memory}). This answer the \textit{RQ2}.
%
...
\end{table}
%
We have reported a favorable assessment of the proposed method. Nevertheless, we have also identified four threats to the validity of our results that can further be assessed and it will be future work: (1) improvement of the power reliability analysis: to include loss of load probability or loss of power supply probability, what can make the analysis more accurate; (2) improvement of the system cost analysis: to include criteria as Net Present Cost, or Levelized Cost of Energy, in order to promote the acceptability of investment (or even to include operational and maintenance costs that were not considered at this study); (3) to increase the equipment and manufactures data base: this will increase the complexity of the optimization, however the result will allow improved sizing as well; and (4) to compare the results of our approach with some off-the-shelf software that focuses on sizing and optimization: this will validate the results and a simulation-verification comparative.
%
