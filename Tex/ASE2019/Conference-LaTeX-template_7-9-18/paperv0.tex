...
it is used to formally check whether an intricate property hols for a given software system. 
Our main focus here is on using software verifiers to formally synthesize optimal sizing of stand-alone solar photovoltaic (PV) systems. 
There exist various state-of-the-art tools to evaluate PV designs but  they are mainly based on simulations, which do not cover all aspects of the design-space. 
...
Commercial equipment data is part of our specification and the technical solution is derived 
...
Experimental results show that our method allied to a software verifier (CPAchecker) 
is able to synthesize,in an acceptable run-time, the optimal PV system sizing of two different case studies from a list of $12$ 
different equipment.
\end{abstract}
