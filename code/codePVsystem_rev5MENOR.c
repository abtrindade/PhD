...
int main(){
	...
	//SOC = nondet_uint()%100; 
	//start considering Batteries FULL
	SOC = 100;

	// not necessary assert (SOC >= SOClimit);

	//calculus of remaining capacity from the batteries in Ah, corrected in terms of charge controller setup of SOC limit
	RemCap = (SOC - SOClimit)*bankcapacity/100; 
	...
	SOC = SOC - 100*drainedCurrent*16/bankcapacity; //decreasing the SOC in 16 hours 
	...
	for (count=1; count <=8; count++){
		...
		if (SOC <= SOClimit) {//battery empty (<=75% of SOC)
			...
			if (Varray >= Vbulk){ //PV panel has enough power to charge the battery AND to supply energy to the load
				...
				SOC = SOC - 100*drainedCurrent*1/bankcapacity;
			}
		}
	...
	//calculus of remaining capacity from the batteries, corrected in terms of charge controller setup of SOC limit
	RemCap = (SOC - SOClimit)*bankcapacity/100; 
	...
	assert (SOC >= SOClimit);  //***new code line when considering that the first 8h count to the 48h autonomy*******
