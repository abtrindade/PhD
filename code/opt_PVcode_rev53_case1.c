...
//must define
float MaxCost=10000; //10 million maximum cost just to control the loop of main function
...
//	__VERIFIER_assume((cost-500) <= (PanelCost+BatteryCost+ControllerCost+InverterCost));
//	__VERIFIER_assume(cost > (PanelCost+BatteryCost+ControllerCost+InverterCost));
	...
	__VERIFIER_assume((cost-500) < Fobj);
