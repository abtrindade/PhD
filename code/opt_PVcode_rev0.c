...
int Faux (int cost){
	...
	BatteryChoice = nondet_uint();
	__VERIFIER_assume((BatteryChoice >= 0) && (BatteryChoice <= BatteryQuant));

	ControllerChoice = nondet_uint();
	__VERIFIER_assume((ControllerChoice >= 0) && (ControllerChoice <= ControllerQuant));

	InverterChoice = nondet_uint();
	__VERIFIER_assume((InverterChoice >= 0) && (InverterChoice <= InverterQuant));
