/* --------------------------------------------------------------------------------------------------------
  Here is the code that performs the automated verification of a stand-alone solar photovoltaic system CAV2019
...
//the main funcion of logarithm of natural base. It calls logchecker1 or checker2.
//The natural logarithm function ln(x) is defined only for x>0. So the natural logarithm of a negative number is undefined
double logchecker (double x){
	double answer;
	if (x > 1.0) answer = logchecker1(x);
	else if (x>0.0) answer=logchecker2(x);
	else answer = NAN;
	return answer;
}

//function that calculates the exponential checker in substitution of EXP from language C
...
void pvmodel (int Gf, int AMBtemperaturef){
	...
	a = qkb*(Vmref-Vocref)/(N*Tk*logchecker(1-Imref/Iscref));

	//Io = reverse saturation current of the diode
	Io = (Iscref+mii*(Tk-Tref))/(expchecker (qkb*(Vocref+miv*(Tk-Tref))/(a*N*Tk)) - 1);
	...
	for (V=0; V<= (Vocref+1); V=V+0.2){
		...
		I = Iph - Io*(expchecker(V/(N*a*VT)) - 1);
...
//perform the size check of solar PV system based on critical month method
int sizing_check(void){
	...
	//battery check
	Cbank = Ecorrected * auton / (Vsystem*(float)(100-SOClimit));
	NBSmin = Vsystem/Vbat;
	assert (NBS >= NBSmin);
...
int main(){
	...
	//OLD item *****2 here, including step 2.1 and 2.2, and 3

	//NOW without PV generation (not VERIFICATION, just CHECK  by code), from 16:00h to 7:00h
...
//step 2.1: from 16h to 24h
	...
	if (auton >= 9) {
		countlimit = 7;   //means go until 7:00h
