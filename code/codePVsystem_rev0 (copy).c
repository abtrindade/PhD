...
int main(){
	...
	SOC = rand()%100;  //rand()%(max-min)+min to number between min and max
	...
	for (count=1; count <=8; count++){
		...
		AMBtemperature = nondet_intXXX; //ambient temperature
	
		//Non-deterministic variable: from 0h to 7:58h is 0 ; 
		//from 8h to 15:59h varies from 274 to 852 W/m2 ; 
		// from  16h to 23:59 is 0
		G = nondet_intYYY; //solar irradiance from SUN in W/m2
	
		// HERE THE CODE FROM PV PANELS
		// Output is: Varray, Iarray, and Parray
		...
		if (SOC <= SOClimit) {//battery empty (<=75% of SOC)
			assert (Vmax >= Vbulk); //if not, the battery is empty and the PV panels are not generating power ***FAIL***
		}
		else if (SOC <= SOCabsorption) { //battery is in absortion (<95) and needs bulk stage of charging, with 14.40V
			if (Vmax >= Vbulk){ //PV panel has enough power to charge the battery AND to supply energy to the load
				Vdc=Vbulk;
			...
			if ((Vmax >= Vfloat) && (SOC < SOCfull)){ 	//PV panel has enough power to charge the battery AND 
