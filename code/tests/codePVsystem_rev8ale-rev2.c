...
//battery of 220 Ah capacity, and minimum SOC (before desconnect load) 75% (i.e., 25% of DOD) as usual to lead-acid batteries
#define NBS 4
#define NBP 1
#define capacity 120
...
//PV system autonomy concerning batteries: 6h
#define auton 6
...
//definition of array arrangement of solar panels
...
int main(){
	unsigned char SOC;
     	int count, AMBtemperature, G, demand[]={45,16,16,16,16,16,0,0,0,72,72,222,150,150,0,0,72,72,814,814,814,742,742,16};
	...
	//SOC = __VERIFIER_nondet_uint()%100; 
	//start considering Batteries not empty
	SOC = 80; 
	...
	for (count=8; count <=15; count++){
		...
		AMBtemperature = __VERIFIER_nondet_uint(); //ambient temperature
		...
		G = __VERIFIER_nondet_uint(); //solar irradiance from SUN in W/m2
	...
	//****2- here the next 6h without PV generation (not VERIFICATION, just CHECK  by code)
	//decreasing the SOC in 6 hours
	assert(SOC>=SOClimit && SOC<=100);
