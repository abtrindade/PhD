/*
 ...
 Name        : PVcode.c
 Author      : Alessandro
 Version     : rev3 - FEB 2020
 Description : New sizing code Prof Wilson (Universal: CBMC, ESBMC, UAutomizer and CPChecker)
...
float Imp = 6.98;
float np = 0.1672;
float Insol = 3.80; //(kWh/m2/day CRESESB 2016) worst month
...
void sizing_check(void){
	...
	float Pminpanels, ItotalPVpanels, VtotalPVpanels, Eb, DODdaycalc, IminDCbus;
...
if( ! (Pminpanels <= (NPS*NPP*Pmref)) ){ __VERIFIER_error();};
//checking voltage and current from the PV panel and the controller
ItotalPVpanels = NPP * Imref;
if( ! (IC >= ItotalPVpanels) ){ __VERIFIER_error();};

VtotalPVpanels = NPS * Vmref;
if( ! (VCmax >= VtotalPVpanels) ){ __VERIFIER_error();};
